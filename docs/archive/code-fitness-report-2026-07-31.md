...
>
> World Bible record-resolution orchestration then moved into `useWorldBibleRecordResolution`: rename and alias suggestions, canonicalization handoffs, duplicate-match decisions, and category moves with search/memory reindexing. `WorldBibleRoute.tsx` fell from 3,215 to 3,011 lines; the new hook is 375 lines. No markup or styling changed, and the same full verification battery passes.
