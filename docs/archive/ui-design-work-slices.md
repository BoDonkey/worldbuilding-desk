...
**Scope:** `apps/web` only. The completed architecture extractions are preserved in `docs/archive/fitness-100-work-slices.md`; this plan should be executed against the current, post-extraction files.
...
Phases 0 and 1 (Slices 1–5) touch only shared components/hooks and are safe to run before or alongside anything else. Slice 3 must land before Slice 4 (Slice 4 imports what Slice 3 builds); Slice 5 can reuse Slice 3's `InlineAlert` but isn't strictly blocked by Slice 4. The fitness-plan extractions that formerly overlapped Slices 7 and 8 are complete and archived; use the current Compendium and Character Sheets component boundaries. Slice 9 (search/filter) touches `CompendiumRoute.tsx` again, so sequence it after Slice 7 if both are in scope. Slice 10 (nav rework) is independent of everything else and can run anytime.
