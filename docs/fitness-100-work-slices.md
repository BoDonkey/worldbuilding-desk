...
The July 24 dependency-security pass exists only as uncommitted working-tree changes: deleted `package-lock.json` at root, `apps/desktop/`, `packages/rules-engine/`; tiptap deps moved from root `package.json` into `apps/web/package.json`; hardened `apps/web/proxy-server.ts`; pnpm overrides in root `package.json`; updated `pnpm-lock.yaml`; doc updates; new `docs/code-fitness-report-2026-07-24.md`.

**Prompt:**

> In the worldbuilding-desk repo (pnpm monorepo), the working tree on branch `codex/ci-trust-gates` contains an uncommitted dependency-security pass. Review `git status` and `git diff` to confirm the changes match this description: three `package-lock.json` files deleted (root, apps/desktop, packages/rules-engine); `@tiptap/*` deps moved from root package.json into apps/web/package.json; root package.json gains pnpm overrides for qs/path-to-regexp/body-parser; `apps/web/proxy-server.ts` hardened (binds 127.0.0.1, restricted CORS, input validation, try/catch); pnpm-lock.yaml updated; docs updated including the new `docs/code-fitness-report-2026-07-24.md`. Do not include unrelated files (e.g. `.claude/` should be gitignored, not committed — add it to `.gitignore` if absent). Before committing, verify: `pnpm install`, then `pnpm --filter web lint`, `pnpm --filter web test:unit`, `pnpm --filter @litrpg-tool/rules-engine test`, `pnpm --filter web build`, `pnpm --filter desktop build`. Run `pnpm audit --prod` and confirm ~13 vulnerabilities remain, all in the protobufjs/sharp families via `@xenova/transformers`. Commit as one or two logical commits (hygiene vs. proxy hardening) with messages referencing the July 24 fitness report, and push.
