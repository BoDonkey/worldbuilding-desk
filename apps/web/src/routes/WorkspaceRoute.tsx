...
function WorkspaceRoute() {
  ...
  const [pendingInventoryCapture, setPendingInventoryCapture] = useState<{
    itemName: string;
    position: number;
  ...
  const resolvedStateMutationEvents = useMemo(
    () =>
      stateMutationEvents.flatMap((event) => {
        ...
        return [
          resolution
            ? {...event, scenePosition: resolution.position}
            : event
        ];
      }),
  ...
  const openSelectionInventoryCapture = useCallback((input: {
    itemName: string;
    from: number;
    to: number;
  }) => {
    ...
    if (!anchor.before && !anchor.after) {
      setFeedback({tone: 'error', message: 'Select item text inside the saved scene first.'});
      return;
    }
  ...
  const saveSelectionInventoryCapture = useCallback(async (input: {
    sheetId: string;
    itemName: string;
    quantity: number;
  }) => {
    ...
    if (!sheet) return;
    const sceneOrder = sortWritingDocuments(documents).findIndex(
    ...
    const event: StateMutationEvent = {
      ...
      sceneOrder,
      sceneSequence: stateMutationEvents
    ...
    try {
      ...
      setFeedback({
        tone: 'success',
        message: `Added ${input.itemName}${input.quantity > 1 ? ` ×${input.quantity}` : ''} to ${sheet.name}'s inventory.`
      });
    } catch (error) {
      setFeedback({
        tone: 'error',
        message: error instanceof Error ? error.message : 'Unable to add item to inventory.'
      });
    } finally {
  ...
  const sceneRosterTimeline = useMemo<SceneRosterTimelineEvent[]>(() => {
    ...
    const expiredSourceIds = new Set(
      stateMutationEvents
        .filter((event) => event.status === 'accepted' && event.consumableEffect?.phase === 'expire')
        .flatMap((event) => event.consumableEffect?.sourceEventId ? [event.consumableEffect.sourceEventId] : [])
    );
    return stateMutationEvents
      .filter(
        (event) =>
          event.sceneId === selectedDocument.id &&
      ...
      .map((event) => {
        ...
        return {
          id: event.id,
          label: event.label?.trim() || `Scene change ${event.sceneSequence ?? ''}`.trim(),
  ...
  const recordSceneRosterChangeHere = useCallback(
    ...
    [content, sceneCursorAnchor, selectedDocument]
  );
  const consumeSceneRosterItemHere = useCallback((
    character: SceneRosterCharacterCard,
    item: SceneRosterInventoryLine
  ) => {
    if (!character.sheetId || !item.consumable || !selectedDocument) return;
    if (selectedDocument.content !== content) {
      setFeedback({tone: 'error', message: 'Save this scene before consuming an item at the cursor.'});
      return;
    }
    if (!sceneCursorAnchor.before && !sceneCursorAnchor.after) {
      setFeedback({tone: 'error', message: 'Place the cursor after the character consumes the item.'});
      return;
    }
    const entry = compendiumEntries.find((candidate) => candidate.id === item.consumable?.definitionId);
    if (!entry?.consumable) return;
    const sheet = characterSheets.find((candidate) => candidate.id === character.sheetId);
    if (!sheet) return;
    const actorId = sheet.characterId ?? sheet.id;
    setPendingPositionedChange({
      character,
      initialLabel: `Consumes ${item.name}`,
      initialCommands: buildConsumableCommands({actorId, itemName: item.name, definition: entry.consumable}),
      consumableEffect: {
    ...
    setSceneRosterStateMoment('cursor');
  }, [characterSheets, compendiumEntries, content, sceneCursorAnchor, selectedDocument]);

  const expireSceneRosterTimelineEvent = useCallback((eventId: string) => {
    const sourceEvent = stateMutationEvents.find((event) => event.id === eventId);
    if (!sourceEvent?.consumableEffect || !selectedDocument) return;
    if (selectedDocument.content !== content) {
      setFeedback({tone: 'error', message: 'Save this scene before placing the expiration.'});
      return;
    }
    if (!sceneCursorAnchor.before && !sceneCursorAnchor.after) {
    ...
    const entry = compendiumEntries.find((candidate) => candidate.id === sourceEvent.consumableEffect?.definitionId);
    const actorId = sourceEvent.commands[0]?.actorId;
    const sheet = characterSheets.find((candidate) => candidate.id === actorId || candidate.characterId === actorId);
    const character = sceneRosterModel.characters.find((candidate) => candidate.sheetId === sheet?.id);
    if (!entry?.consumable || !actorId || !character) {
      setFeedback({tone: 'error', message: 'Add the affected character to the scene roster before placing the expiration.'});
      return;
    }
    setPendingPositionedChange({
      character,
      initialLabel: `${sourceEvent.consumableEffect.itemName} expires`,
      initialCommands: buildConsumableExpirationCommands({actorId, definition: entry.consumable}),
      consumableEffect: {
        definitionId: entry.id,
    ...
    setSceneRosterStateMoment('cursor');
  }, [characterSheets, compendiumEntries, content, sceneCursorAnchor, sceneRosterModel.characters, selectedDocument, stateMutationEvents]);
  const editSceneRosterTimelineEvent = useCallback(
    (eventId: string) => {
      ...
      if (!sheet || !character) {
        setFeedback({tone: 'error', message: 'Add this character to the scene roster before editing the change.'});
        return;
      }
      const resolution = stateMutationAnchorResolutionById.get(event.id);
  ...
  const invalidateSceneRosterTimelineEvent = useCallback(async (eventId: string) => {
    const event = stateMutationEvents.find((entry) => entry.id === eventId);
    ...
    if (updated) {
      setFeedback({tone: 'success', message: `Invalidated “${event.label || 'scene change'}”.`});
    }
  }, [stateMutationEvents]);
  const reanchorSceneRosterTimelineEvent = useCallback(async (eventId: string) => {
    const event = stateMutationEvents.find((entry) => entry.id === eventId);
    if (!event || !selectedDocument) return;
    if (selectedDocument.content !== content) {
      setFeedback({tone: 'error', message: 'Save the scene before re-anchoring this change.'});
      return;
    }
    if (!sceneCursorAnchor.before && !sceneCursorAnchor.after) {
      setFeedback({tone: 'error', message: 'Place the cursor beside the intended scene text first.'});
      return;
    }
    try {
      ...
      setFeedback({
        tone: 'success',
        message: `Re-anchored “${event.label || 'scene change'}” at the current cursor.`
      });
    } catch (error) {
      setFeedback({
        tone: 'error',
        message: error instanceof Error ? error.message : 'Unable to re-anchor scene change.'
      });
    }
  }, [content, sceneCursorAnchor, sceneCursorPosition, selectedDocument, stateMutationEvents]);

  const pendingPositionedSheet = useMemo(
    () =>
      pendingPositionedChange?.character.sheetId
        ? characterSheets.find(
            (candidate) => candidate.id === pendingPositionedChange.character.sheetId
          ) ?? null
        : null,
  ...
  const positionedChangeBefore = useMemo(() => {
    ...
    return replayCharacterState({
      ...
      ruleset,
      events: resolvedStateMutationEvents.filter(
        (event) => {
  ...
  const savePositionedChange = useCallback(async (input: {
    label: string;
    ...
    const event: StateMutationEvent = {
      ...
      consumableEffect: pendingPositionedChange.consumableEffect ?? existingEvent?.consumableEffect,
    ...
    try {
      ...
      setFeedback({
        ...
        message: existingEvent
          ...
          : `Recorded “${input.label || 'scene change'}” at cursor position ${sceneCursorPosition}.`
      });
    } catch (error) {
      setFeedback({
        tone: 'error',
  ...
  const getCharacterStateHoverCard = useCallback((
    ...
    const stats = Object.entries(replayed.stats)
      ...
      .map(([statId, value]) => {
        ...
        const effective = typeof value === 'number'
          ? getEffectiveStatValue({
              definitionId: statId,
              baseValue: value,
              runtime: runtimeModifiers
