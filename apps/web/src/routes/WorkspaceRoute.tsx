...
function WorkspaceRoute({activeProject, projectSettings: initialProjectSettings = null}: WorkspaceRouteProps) {
  const workspaceDebugWindow = window as WorkspaceDebugWindow;
  ...
  useEffect(() => {
    workspaceDebugWindow.__wbdWorkspaceMountedAt = Date.now();
    workspaceDebugWindow.__wbdWorkspaceRenderCount = 0;
    return () => {
      workspaceDebugWindow.__wbdWorkspaceUnmountedAt = Date.now();
    };
  }, [workspaceDebugWindow]);

  // Lightweight diagnostics only, no state updates.
  workspaceDebugWindow.__wbdWorkspaceRenderCount =
    (workspaceDebugWindow.__wbdWorkspaceRenderCount ?? 0) + 1;
